...
public class Photo {
	...
	public void renamePhoto(File newName) throws SecurityException, IOException{
		FileHandler myfh = new FileHandler("C:/Users/July Ju/Desktop/uoftyear2/csc207/group_0633/a2new/renaminglogfile.log");
		SimpleFormatter myfmt = new SimpleFormatter();
		MySystem.renamingLog.addHandler(myfh);
		myfh.setFormatter(myfmt);
		MySystem.renamingLog.info("OldName is: " + this.name.getName() + " , " + "NewName is: " + newName.getName());
		this.name = newName;
		nameLog.add(newName);
	...
	public void addTag(Tag newTag) throws SecurityException, IOException{
		...
		File newFileName = new File("@" + newTag.getName() + this.name.getName());
...
//		ArrayList<Tag> newTagList = new ArrayList<Tag>(this.tagList);
		...
		this.tagList.remove(deletedTag);
		for(Tag element: tagList){
			newName = "@" + element.getName() + newName;
		}
